...
test("Não deve criar um pedido com cpf inválido", function () {
	...
	expect(() => new Order(uuid, new Customer("Cliente 1", new Cpf(cpf)))).toThrow(new Error("Invalid cpf"));
});

test("Deve criar um pedido vazio", function () {
	const uuid = crypto.randomUUID();
	const cpf = "407.302.170-27";
	const order = new Order(uuid, new Customer("Cliente 1", new Cpf(cpf)));
...
test("Deve criar um pedido com 3 itens, sendo um em dólar", function () {
	...
	const order = new Order(uuid, new Customer("Cliente 1", new Cpf(cpf)), currencyTable);
	order.addItem(new Product(1, "A", 1000, 100, 30, 10, 3, "BRL"), 1);
...
test("Deve criar um pedido e gerar o código", function () {
	const uuid = crypto.randomUUID();
	const cpf = "407.302.170-27";
	const order = new Order(uuid, new Customer("Cliente 1", new Cpf(cpf)), new CurrencyTable(), 1, new Date("2023-10-01T10:00:00"));
